function animate(e) {
  if (e.target.parentElement.parentElement.id === "lists-content") {
    if (e.target.hasAttribute("name")) {
      ...
      document.getElementById("co-counter").innerHTML =
        Number(document.getElementById("co-counter").innerHTML) + 1;
      document.getElementById("li-counter").innerHTML =
        Number(document.getElementById("li-counter").innerHTML) - 1;
      ...
      let d = e.target.parentElement;
      let initial = document.querySelectorAll("#lists-content i.fa-regular")[1]
        .parentElement;
      if (initial !== null) {
        document.getElementById("lists-content").insertBefore(d, initial);
      }
    ...
    if (e.target.hasAttribute("name")) {
      ...
      e.target.setAttribute("class", "fa-regular fa-star");
      let d = e.target.parentElement;
      let initial = document.querySelectorAll(
        "#completed-content i.fa-regular"
      )[1].parentElement;
      if (initial !== null) {
        document.getElementById("completed-content").insertBefore(d, initial);
      }
...
document.getElementById("lists-header").addEventListener("click", () => {
  ...
  if (content === "" || content === "none") {
    document
      .querySelector("#li.fa-angle-right")
      .setAttribute("class", "fa fa-angle-down");
    document.getElementById("lists-content").style.display = "block";
  } else {
    document
      .querySelector("#li.fa-angle-down")
      .setAttribute("class", "fa fa-angle-right");
